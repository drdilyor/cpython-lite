<template>
  ...
  <slot v-else-if="d.response !== null && d.response.some(i => !i.ok)" name="error">
    <api-view-error :response="d.response.find(i => !i.ok)" :data="d.data" :fetchData="fetchData" />
  </slot>
...
const fetchData = () => {
  ...
  d.error = null
  d.data = {}
  Promise.all(props.url.map(i => api.get(i)))
  .then(res => {
    d.response = res;
